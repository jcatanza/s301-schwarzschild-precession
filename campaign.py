...
diffraction limit is larger than S301's whole orbit; fixed here):
  ...
  1. Astrometric offset (RA, Dec, mas) -- GRAVITY+/VLTI.
     constants.GRAVITY_PLUS_ASTROMETRY_MAS (207 uas) is the per-epoch
     precision ACHIEVED on S301 in the discovery data (their pixel-grid
     noise floor). The discovery paper's GRAVITY+ forecast of 100 uas
     ("as expected for the final performance of GRAVITY+") is the
     optimistic case in precision_sensitivity.py, which reports the
     result at both.
...
def report_and_write_csv(epochs, outpath, label="heuristic"):
    ...
    print(f"Injected Schwarzschild precession: {omega_dot_deg_yr:.4f} deg/yr "
          ...
          f"not illustrative")
    print(f"Astrometric precision (GRAVITY+, achieved on S301 in the discovery data): "
